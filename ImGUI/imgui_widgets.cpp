...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(pos, pos + ImVec2(square_sz + 20.f, square_sz));
    const ImRect frame_bb(pos, pos + ImVec2(square_sz, square_sz));
    RenderNavHighlight(total_bb, id);
    RenderFrame(check_bb.Min, check_bb.Max, ImColor(35, 35, 35, 255), true, style.FrameRounding);
    RenderFrame(*v ? frame_bb.Min + ImVec2(20, 0) : frame_bb.Min,
        *v ? frame_bb.Max + ImVec2(20, 0) : frame_bb.Max,
        *v ? ImColor(237, 58, 57, 255) : ImColor(255, 255, 255, 80),
...
bool ImGui::ColorEdit3(const char* label, float col[3], bool* rainbow, ImGuiColorEditFlags flags)
{
    return ColorEdit4(label, col, rainbow, flags | ImGuiColorEditFlags_NoAlpha);
}

// Edit colors components (each component in 0.0f..1.0f range).
// See enum ImGuiColorEditFlags_ for available options. e.g. Only access 3 floats if ImGuiColorEditFlags_NoAlpha flag is set.
// With typical options: Left-click on color square to open color picker. Right-click to open option menu. CTRL-Click over input fields to edit them and TAB to go to next item.
bool ImGui::ColorEdit4(const char* label, float col[4], bool* rainbow, ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            ...
            value_changed |= ColorPicker4("##picker", col, rainbow, picker_flags, &g.ColorPickerRef.x);
            EndPopup();
        }
    }
...
bool ImGui::ColorPicker3(const char* label, float col[3], bool* rainbow, ImGuiColorEditFlags flags)
{
    float col4[4] = { col[0], col[1], col[2], 1.0f };
    if (!ColorPicker4(label, col4, rainbow, flags | ImGuiColorEditFlags_NoAlpha))
        return false;
...
bool ImGui::ColorPicker4(const char* label, float col[4], bool* rainbow, ImGuiColorEditFlags flags, const float* ref_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (!(flags & ImGuiColorEditFlags_NoOptions))
        ColorPickerOptionsPopup(col, rainbow, flags);
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            if (ColorEdit4("##rgb", col, rainbow, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            value_changed |= ColorEdit4("##hsv", col, rainbow, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            value_changed |= ColorEdit4("##hex", col, rainbow, sub_flags | ImGuiColorEditFlags_DisplayHex);
        PopItemWidth();
    }
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, bool* rainbow, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            ColorPicker4("##previewing_picker", &previewing_ref_col.x, rainbow, picker_flags);
            PopID();
        }
        PopItemWidth();
    }
